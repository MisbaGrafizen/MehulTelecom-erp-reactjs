...
import { X, Printer, Download, Calendar, Package, Smartphone } from 'lucide-react'
...
export default function PurchaseModal({ purchase, onClose }) {
  ...
  return (
    ...
    >
      ...
      >
        {/* Header */}
        <div className="sticky top-0 flex items-center justify-between p-2 border-b border-border  bg-gray-200">
          <div className="flex-1">
            <h2 className="text-2xl font-bold text-foreground mb-2">{purchase.invoiceNo}</h2>
            <div className="flex gap-2 flex-wrap">
              <Badge type="purchase" text="Purchase" />
              <Badge type="completed" text="Completed" />
              <span className="inline-block px-3 py-1 rounded-full text-xs font-medium bg-green-light text-green-paid flex items-center gap-1">
                <Calendar size={12} />
                {purchase.time}
              </span>
            </div>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            >
              ...
              <div className="space-y-3">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-semibold text-foreground">{purchase.invoiceNo}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground">Payment Mode</span>
                  <span className="font-semibold text-foreground">{purchase.paymentMode}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground">Items Count</span>
                  <span className="font-bold text-primary text-base">{purchase.details.items}</span>
                </div>
                <div className="flex justify-between text-sm pt-2 border-t border-border">
                  <span className="text-muted-foreground">Total Amount</span>
                  <span className="font-bold text-green-paid text-base">{purchase.amount}</span>
                </div>
              </div>
            </motion.div>
            ...
            <motion.div
              ...
              className=" bg-gray-200 border border-border rounded-xl p-6 space-y-4"
            >
              <h3 className="font-bold text-foreground">Party Information</h3>
              <div className="space-y-3 text-sm">
                <div>
                  <p className="text-muted-foreground text-xs uppercase tracking-wide mb-1">Company</p>
                  <p className="font-semibold text-foreground">{purchase.details.company}</p>
                </div>
                <div>
                  <p className="text-muted-foreground text-xs uppercase tracking-wide mb-1">Supplier</p>
                  <p className="font-semibold text-foreground">{purchase.details.partyName}</p>
                </div>
              </div>
            </motion.div>
          </div>
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex gap-3 flex-1">
                      <div className="w-12 h-12 rounded-lg bg-gradient-to-br from-green-paid to-green-600 flex items-center justify-center flex-shrink-0">
                    ...
                    <span className="px-3 py-1 rounded-full bg-green-light text-green-paid text-xs font-bold flex-shrink-0">
                      Qty: {item.qty}
                    </span>
                  </div>
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            >
              ...
              <p className="text-2xl font-bold text-green-paid">{purchase.details.items}</p>
            </motion.div>
            ...
            >
              <p className="text-muted-foreground text-xs uppercase tracking-wide mb-2">Payment</p>
              <p className="text-sm font-bold text-blue-unpaid">{purchase.paymentMode}</p>
            </motion.div>
            ...
            >
              ...
              <p className="text-lg font-bold text-green-paid">{purchase.amount}</p>
            </motion.div>
          </div>
          ...
          >
            ...
            <div className="flex gap-3">
              ...
              <motion.button
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-green-paid to-green-600 text-white rounded-lg hover:opacity-90 transition-opacity font-medium text-sm"
              >
                <Download size={16} />
                Download
              </motion.button>
