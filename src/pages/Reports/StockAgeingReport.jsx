...
export function StockAgeingReport() {
 ...
 const [summary, setSummary] = useState({
  ...
  const [filterOptions, setFilterOptions] = useState({
     stockTypes: ["All Stock", "Mobiles", "Watches", "Accessories"],
     ageFilters: ["7 Days", "15 Days", "30 Days", "60 Days", "90 Days", "180 Days"],
 });
 ...
 const fetchAgeingData = async () => {
   try {
     const query = new URLSearchParams({
       stockType: filters.stockType || "All Stock",
       ageFilter: filters.ageFilter || "30 Days",
       fromDate: filters.fromDate || "",
       toDate: filters.toDate || "",
       search: filters.search || "",
     }).toString();

     const res = await ApiGet(`/admin/stock-ageing?${query}`);
     console.log("AGEING REPORT RESPONSE:", res);

     setStockData(res.items || []);
     setSummary(res.summary || {});

     // Filter dropdown values coming from backend
     setFilterOptions({
       stockTypes: ["All Stock", ...(res.stockTypes || [])],
       ageFilters: ["7 Days", "15 Days", "30 Days", ...(res.ageFilters || [])],
     });

   } catch (error) {
     console.log("AGEING SUMMARY FETCH ERROR:", error);
   }
 };
 ...
 <KpiCards 
     ...
     highAgeingStock={summary.highAgeingStock}
     oldStock={summary.oldStock}
 />

                                {/* Stock Listing */}
 <StockAgeList 
     data={stockData} 
     onViewClick={setSelectedStock} 
 />
                            </div>
